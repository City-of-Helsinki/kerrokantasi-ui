...
| 2.4 | `react-router-dom` | 6.30.3 | v6 peer allows React 18 only; v7 supports 19 | **Upgrade** → `^7.x` (recommended) **or** keep v6 with peer override | v7 is mostly drop-in for v6; rename a few imports (`Routes`, `Outlet` unchanged). Audit ~30 call sites. |
...
| 2.16 | `react-router-hash-link` | 2.4.3 | Peer `react-router-dom@^6` | **Replace** if moving to react-router v7 | Used only in `src/components/LinkWithLang.jsx`. Trivial to inline. |
