...
const defaultProps = {
  ...
  profile: {
    ...
    favoriteHearings: {
      count: 2,
      results: [{ id: 'firstFavoriteHearing' }, { id: 'secondFavoriteHearing' }],
    },
  },
};

const commentCounter = (uniqueHearing) =>
  defaultProps.profile.comments.uniqueHearings.find((value) => value.id === uniqueHearing.id).commentCount;
  ...
  describe('renders', () => {
    ...
    describe('main FormattedMessage components', () => {
      test('with correct ids and values if used', () => {
        ...
        expect(messageElements.filterWhere((element) => element.prop('id') === 'userInfo')).toHaveLength(1);
        expect(messageElements.filterWhere((element) => element.prop('id') === 'favoriteHearings')).toHaveLength(1);
