...
describe('HearingFormStep3', () => {
  ...
  describe('methods', () => {
    describe('onDrawCreated', () => {
      ...
      test(
        'calls onAddMapMarker when adding an element to a hearing with a single geojson that has already been edited',
        () => {
          // ie isEdited is true
          const mockOnAddMapMarker = jest.fn();
          const mockEvent = getEvent(mockAddedPoint);
    ...
    describe('onDrawDeleted', () => {
      ...
      test(
        'call onHearingChange with correct params when removing the last/only element(not collection) of a hearing',
        () => {
