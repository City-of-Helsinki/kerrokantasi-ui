...
describe('HearingEditor actions', () => {
    ...
    it('fetches metadata and dispatches RECEIVE_META_DATA', async () => {
      ...
      api.getAllFromEndpoint.mockImplementation((endpoint) => {
        if (endpoint === '/v1/label/')
          return Promise.resolve(['Label 1', 'Label 2']);
        if (endpoint === '/v1/organization/')
          return Promise.resolve(['Org 1', 'Org 2']);
  ...
  describe('Deletion Actions', () => {
    it('deletes a section attachment and dispatches DELETE_ATTACHMENT', async () => {
      ...
      await store.dispatch(
        actions.deleteSectionAttachment(sectionId, attachment)
      );
      expect(store.getActions()).toEqual(expectedActions);
    });
  ...
  describe('Modification Actions', () => {
    ...
    it('dispatches EDIT_QUESTION when editing a question', () => {
      ...
      store.dispatch(
        actions.editQuestion(fieldType, sectionId, questionId, optionKey, value)
      );
      expect(store.getActions()).toContainEqual(expectedAction);
    });
  });
  describe('saveHearingChanges', () => {
    it('dispatches SAVE_HEARING_SUCCESS and navigates on successful save', async () => {
      ...
      const response = {
        status: 200,
        json: () => Promise.resolve(hearingJSON),
      };
      api.put.mockResolvedValue(response);

      const expectedActions = [
        {
          type: EditorActions.SAVE_HEARING,
          payload: { cleanedHearing: mockProcessedHearing },
        },
        {
          type: EditorActions.SAVE_HEARING_SUCCESS,
          payload: { hearing: hearingJSON },
        },
      ];
      ...
      const expectedActions = [
        addToast(
          createNotificationPayload(
            ...
            'Sinulla ei ole oikeutta muokata yhteyshenkilöä.'
          )
        ),
      ];
  ...
  describe('addLabel', () => {
    ...
    it('dispatches ADD_LABEL_SUCCESS on successful label addition', async () => {
      api.post.mockResolvedValue({
        status: 201,
        json: () => Promise.resolve(responseLabel),
      });
      ...
      const expectedActions = [
        { type: EditorActions.ADD_LABEL },
        {
          ...
          payload: { label: responseLabel },
        },
        {
          type: EditorActions.EDIT_HEARING,
          payload: {
            field: 'labels',
            value: [...mockSelectedLabels, responseLabel.id],
          },
        },
        addToast(
          createNotificationPayload(
    ...
    it('dispatches ADD_LABEL_FAILED on failure (400)', async () => {
      ...
      const expectedActions = [
        ...
        addToast(
          createNotificationPayload(
            NOTIFICATION_TYPES.error,
            'Tarkista asiasanan tiedot.'
          )
        ),
        {
          type: EditorActions.ADD_LABEL_FAILED,
          payload: { errors: errorResponse },
        },
      ];
  ...
  describe('saveAndPreviewHearingChanges', () => {
    it('dispatches SAVE_HEARING and SAVE_HEARING_SUCCESS on successful save', async () => {
      const response = {
        status: 200,
        json: () => Promise.resolve(mockHearing),
      };
      ...
      const filteredActions = dispatchedActions.map(
        ({ meta, ...action }) => action
      );
