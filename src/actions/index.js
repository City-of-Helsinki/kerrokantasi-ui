...
export const requestErrorHandler =
  (dispatch, localizationKey = undefined) =>
  (err) => {
    ...
    let payload;
    if (localizationKey) {
      payload = createLocalizedNotificationPayload(
...
export function fetchUserComments(additionalParams = {}) {
  return (dispatch) => {
    ...
    return apiGet('v1/comment', params)
      .then(getResponseJSON)
      .then((data) => {
...
export const fetchMoreHearings = (listId) => (dispatch, getState) => {
  ...
  return apiGet('v1/hearing/', url.query)
    .then(getResponseJSON)
    .then((data) => {
...
export function fetchLabels() {
  return (dispatch) => {
    ...
    return getAllFromEndpoint('/v1/label/')
      .then((data) => {
...
export function fetchHearing(hearingSlug, previewKey = null) {
  return (dispatch) => {
    ...
    if (
      !hearingSlug ||
      hearingSlug === 'undefined' ||
      typeof hearingSlug !== 'string'
    ) {
      const error = new Error(`Invalid hearing slug: ${hearingSlug}`);
      dispatch(
        createAction('receiveHearingError')({
          hearingSlug: hearingSlug || 'unknown',
        })
      );
...
export function addHearingToFavorites(hearingSlug, hearingId) {
  return (dispatch) => {
    const fetchAction = createAction('beginAddHearingToFavorites')({
      hearingSlug,
    });
    dispatch(fetchAction);
    ...
    return post(url)
      ...
      .then((data) => {
        if (data.status_code === 304) {
          ...
          dispatch(
            createAction('modifyFavoriteHearingsData')({
              hearingSlug,
              hearingId,
            })
          );
...
export function removeHearingFromFavorites(hearingSlug, hearingId) {
  return (dispatch) => {
    const fetchAction = createAction('beginRemoveHearingFromFavorites')({
      hearingSlug,
    });
    dispatch(fetchAction);
    ...
    return post(url)
      .then((data) => {
        if (data.status === 204) {
          dispatch(
            createAction('receiveRemoveHearingFromFavorites')({
              hearingSlug,
              data,
            })
          );
 ...
 */
export const getCommentSubComments =
  (commentId, sectionId, jumpTo) => (dispatch) => {
    ...
    return apiGet(URL_COMMENT, params)
      .then(getResponseJSON)
      .then((data) => {
        dispatch(
...
export function fetchMoreSectionComments(
  ...
  return (dispatch) => {
    ...
    return apiGet('v1/comment/', url.query)
      .then(getResponseJSON)
      .then((data) => {
...
export function fetchAllSectionComments(
  ...
  return (dispatch) => {
    ...
    return apiGet(url, { include: 'plugin_data', ordering })
      .then(getResponseJSON)
      .then((data) => {
        dispatch(createAction('receiveSectionComments')({ sectionId, data }));
      })
    ...
    return post(url, params)
      ...
      .then((data) => {
        if (commentData.comment && typeof commentData.comment !== 'undefined') {
          dispatch(
            getCommentSubComments(commentData.comment, sectionId, data.id)
          );
        } else {
          dispatch(
...
export function editSectionComment(
  ...
  return (dispatch) => {
    ...
    return put(url, params)
      ...
      .then((responseJSON) => {
        dispatch(
          createAction('editedComment')({ sectionId, comment: responseJSON })
        );
        dispatch(fetchHearing(hearingSlug));
    ...
    return post(url)
      ...
      .then((data) => {
        if (data.status_code === 304) {
          ...
          dispatch(
            createAction('postedCommentFlag')({
              commentId,
              sectionId,
              isReply,
              parentId,
