...
describe('api.js', () => {
  ...
  describe('getApiURL', () => {
    it('should return the full API URL with endpoint', () => {
      expect(getApiURL('test-endpoint')).toBe(
    ...
    it('should throw an error if there is a double query string', () => {
      expect(() =>
        getApiURL('test-endpoint?existing=query', { param1: 'value1' })
      ).toThrow('Double query string');
    });
  });
  ...
  describe('getAllFromEndpoint', () => {
    it('should fetch all paginated results', async () => {
      const firstPage = {
        results: [{ id: 1 }],
        next: 'http://example.com/api/test-endpoint/?page=2',
      };
