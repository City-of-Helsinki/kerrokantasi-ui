...
vi.mock('../ExternalContentPlaceholder', () => ({
  default: ({ url }) => (
    <div data-testid='external-content-placeholder'>Placeholder for: {url}</div>
  ),
}));
...
describe('HtmlWithConsentCheck', () => {
  ...
  describe('without iframes', () => {
    it('renders sanitized HTML content without iframes', () => {
      ...
      const { container } = render(
        <HtmlWithConsentCheck htmlString={htmlString} />
      );
  ...
  describe('with iframes and marketing consent', () => {
    it('renders iframes when marketing consent is given', () => {
      ...
      const htmlString =
        '<p>Video:</p><iframe src="https://www.youtube.com/embed/test"></iframe>';
      const { container } = render(
        <HtmlWithConsentCheck htmlString={htmlString} />
      );
  ...
  describe('with iframes and no consent', () => {
    it('replaces iframes with placeholders when no consent', () => {
      ...
      expect(
        screen.getByTestId('external-content-placeholder')
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          /Placeholder for: https:\/\/www.youtube.com\/embed\/test/
        )
      ).toBeInTheDocument();
    });
    ...
    it('handles multiple iframes with surrounding content', () => {
      ...
      expect(
        screen.getAllByTestId('external-content-placeholder')
      ).toHaveLength(2);
    });
  });
  ...
  describe('consent state changes', () => {
    it('updates when consent changes from false to true', () => {
      ...
      expect(container.querySelector('iframe')).toBeInTheDocument();
      expect(
        screen.queryByTestId('external-content-placeholder')
  ...
  describe('edge cases', () => {
    ...
    it('removes iframe-wrapper divs when showing placeholders', () => {
      ...
      const htmlString =
        '<p>Text</p><div class="iframe-wrapper"><iframe src="https://www.youtube.com/embed/test">' +
        '</iframe></div><p>More text</p>';
    ...
    it('maintains correct HTML structure with wrapper replacement', () => {
      ...
      const htmlString =
        '<div class="content"><p>Paragraph</p>' +
    ...
    it('handles iframes without wrapper correctly', () => {
      ...
      const htmlString =
        '<div><p>Before</p><iframe src="https://www.youtube.com/embed/test"></iframe><p>After</p></div>';
