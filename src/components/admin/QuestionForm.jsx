...
const QuestionForm = ({
  ...
  const handleDeleteExistingQuestion = () => {
    if (question.id && sectionId) {
      onDeleteExistingQuestion(sectionId, question.id);
    }
  };

  const getEditableForm = () => (
    <div className='question-form' key={question.type}>
      ...
      <div>
        <Button
          ...
          onClick={() => addOption(sectionId, question.frontId || question.id)}
        >
  ...
  const getQuestionDetails = () => (
    <div>
      ...
      <div>
        ...
        <Button
          className={'kerrokantasi-btn danger'}
          onClick={handleDeleteExistingQuestion}
        >
  ...
  return question.frontId ||
    (question.id && !isPublic && question.n_answers === 0)
    ? getEditableForm()
    : getQuestionDetails();
};
