...
class LinkWithLangComponent extends React.Component {
  render() {
    const { to, rel, target, className, children, style, headless, location } = this.props;
    ...
    return (
      <Link className={className} to={newTo} rel={rel} target={target} style={style}>{children}</Link>
    );
  }
}
