...
const hearingContainsQuestions = (hearing) =>
  Boolean(hearing.sections.some((s) => s.questions.length));
...
test.describe('Hearing', () => {
  ...
  test('should display Finnish title', async ({ page }) => {
    await expect(page.locator('.hearing-header-title')).toContainText(
      hearing.title.fi
    );
  });
  ...
  test('should display comment count', async ({ page }) => {
    await expect(page.getByTestId('comment-summary')).toContainText(
      `Yhteensä ${hearing.n_comments} kommentti`
    );
  });
  ...
  test('should have Swedish link if available', async ({ page }) => {
    ...
    await expect(page.getByTestId('language-select')).toContainText(
      'Hörandet tillgängligt på svenska'
    );
  });
  ...
  test('should display label', async ({ page }) => {
    ...
    if (firstFinnishLabel) {
      await expect(page.locator('[aria-label="Asiasanat"]')).toContainText(
        firstFinnishLabel
      );
    }
  });
  ...
  test('should display contact person name', async ({ page }) => {
    ...
    await expect(page.locator('.hearing-contacts')).toContainText(
      contactPerson.name
    );
  });
  ...
  test('should have Excel link for comment summary', async ({ page }) => {
    const link = page.locator(
      'a:has-text("Lataa yhteenveto kommenteista Excel-muodossa")'
    );
    await expect(link).toBeVisible();
  });
  ...
  test('verify visibility of questions and their options', async ({ page }) => {
    ...
    await page
      .getByRole('button', { name: 'Äänestä ja kommentoi' })
      .last()
      .click();
    ...
    for (const option of options) {
      await expect(page.getByTestId('question-form-group')).toContainText(
        option
      );
    }
  });
  ...
  test('user can successfully submit a comment', async ({ page }) => {
    ...
    await page
      ...
      .fill('Tämä on selaintestin kirjoittama viesti');
  ...
  test('check Finnish title in first subsection if subsections exist', async ({
    ...
    const hasSectionCard =
      hearing.sections.filter((s) => s.type === 'part').length > 0;
